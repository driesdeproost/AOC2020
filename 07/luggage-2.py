...
def weight(bag: str, rules: dict) -> int:
    content: dict = rules.get(bag)
    return 1 + sum([weight(x, rules) * int(content.get(x)) for x in content.keys()])
...
print(weight(myBag, rules) - 1) # not mybag itself, just what is inside